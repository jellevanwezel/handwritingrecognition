...
\framebox[\width]{
\begin{algorithm}[H]
 \KwData{prototypes, learningRate, data,labels}
 \KwResult{prototypes}
 initialization\;
 \While{prototypes have not converged}{
   \ForAll{data-point $\in$ data}{
      Determine the closest prototype to data-point.\\
      \If {label data-point == label prototype}{
        Move prototype towards data-point by learningRate.
      }
      \Else{
        Move prototype away from data-point by learningRate.
      }
    }
  }
 \caption{The training of the LVQ algorithm}
\end{algorithm}
}
\\\\
Multiple variants have been made on the LVQ algorithm. Sato showed in his paper (ref) \todo{ref} that a generalized version of LVQ 2.1 (ref) \todo{ref} always converges. LVQ 2.1 uses 2 nearest prototypes. One with the same label as the data-point and one with an other label as the data-point. The first prototype is pushed towards the data-point and the second is pushed away from the data-point.

For the experiments Generalized Learning Vector Quantization (GLVQ) and three variations on GLVQ were used. The used GLVQ variations were: Relevance Generalized LVQ (RGLVQ), Generalized Metric LVQ (GMLVQ), and Local Generalized Metric LVQ (LGMLVQ).
...
A slightly altered version of the LVQ toolbox by Biehl (ref) \todo{ref} was used for the implementation of the LVQ algorithms. GMLVQ was altered to be able to run GLVQ because the toolbox did not supply an algorithm for this. This was done by setting the learning rate for the matrix to zero and initialing the matrix to the identity matrix so all features have the same relevance.
