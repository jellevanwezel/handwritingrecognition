...
%T. Kohonen (1995), "Learning vector quantization", in M.A. Arbib, The Handbook of Brain Theory and Neural Networks, Cambridge, MA: MIT Press, pp. 537–540
...
\begin{flalign}
\mu(\vec{x}) = \frac{d_1-d_2}{d_1+d_2}
\end{flalign}


Then the energy function is defined as:
\begin{flalign}
S = \sum_{i=1}^{N} f(\mu(\vec{x}_i))
\end{flalign}
...
\begin{flalign}
\vec{w}_i \leftarrow \vec{w}_i - \alpha \frac{dS}{d\vec{w}_i}, i=1,2
\end{flalign}

\begin{flalign}
\frac{dS}{d\vec{w}_1} = - \frac{df}{d\mu}\frac{4d_2}{(d_1+d_2)^2}(\vec{x}-\vec{w}_1)
\end{flalign}

\begin{flalign}
\frac{dS}{d\vec{w}_2} = + \frac{df}{d\mu}\frac{4d_1}{(d_1+d_2)^2}(\vec{x}-\vec{w}_2)
\end{flalign}

\begin{flalign}
\vec{w}_1 \leftarrow \vec{w}_1 + \alpha \frac{df}{d\mu}\frac{d_2}{(d_1+d_2)^2}(\vec{x}-\vec{w}_1)
\end{flalign}

\begin{flalign}
\vec{w}_2 \leftarrow \vec{w}_2 - \alpha \frac{df}{d\mu}\frac{d_1}{(d_1+d_2)^2}(\vec{x}-\vec{w}_2)
\end{flalign}
...
RGLVQ is an addition by Hammer on GLVQ by Sato,
...
\begin{flalign}
D = | \vec{\lambda} \vec{x}_i - \vec{\lambda} \vec{w}_j |
\end{flalign}
...
\begin{flalign}
\Delta \Omega_{lm} = - \epsilon \cdot 2 \cdot \vec{\Phi}' (\mu(\xi)) \cdot (\mu^+(\xi) ( (\xi_m - \omega_{J,m})[\Omega(\xi-\omega_J)]_l ) - \mu^-(\xi) \cdot ( (\xi_m-\omega_{K,m})[\Omega(\xi-\omega_K)]_l ) ) 
\end{flalign}
