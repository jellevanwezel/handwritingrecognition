...
\framebox[\width]{
\begin{algorithm}[H]
 \KwData{prototypes, data,labels}
 \KwResult{prototypes}
 initialization\;
 1. Calculate the distances from the prototypes to each data-point in data\\
 2. Sort the labels of the data-points ascending by distance to x\\
 3. Take the mode of the top k labels from the sorted set\\ 
...
% generalisation of the algorithm (proof of convergence)
...
Generalized LVQ is the LVQ algorithm as proven to converge by (REFEREFER) \todo{ref} 
...
% normalization (naive,norm,norm_var)
% 
