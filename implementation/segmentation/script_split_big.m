...
for i = 3:10 %size(dirContents,1)    
    ...
    folderDir = dir([dataDir,folderName(1:end-4),'/']);
    ...
    for j = 3:size(folderDir,1)
        ...
        A = imread(strcat(folderName,fileName));
        ...
        outputPath([outputDir,folderName,'/',num2str(j),'/']);
        mkdir(outputPath); 
        
        for k = 1:length(splitted)
            imageSplit =  splitted{k};
            imwrite(I,strcat(outputPath,'split_',num2str(k),'.png'));
        end
    end
end